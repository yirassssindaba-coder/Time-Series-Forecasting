    ...
    if yf is not None:
        ...
        while attempt < max_retries:
            ...
            try:
                ...
                backoff *= 2
    # local fallback
    for candid in [p, out_dir / f"{ticker.upper()}.csv", out_dir / f"{ticker.lower()}.csv"]:
        if candid.exists():
            try:
                df = pd.read_csv(candid, index_col=0, parse_dates=True, engine="python")
                if not df.empty:
                    return df, candid
            except Exception:
                pass
...
<head>
  ...
  <meta name="description" content="Upload image left, create YoY chart right, edit (overlay) chart onto uploaded image and save edited image. If no image present, Edit will create a new base image to[...]
  <style>
    :root{
...
<body>
  <div class="container">
    <section class="card">
      <h2>Plot (PNG / JPG)</h2>
      <div class="small">Upload pipeline PNG/JPG. Use "Edit PNG" to apply chart overlay — the left image will be edited in-place. If no image exists, Edit will create a new white canvas and draw the[...]
    ...
    <aside class="card right-col">
      ...
      <div class="small">Unggah CSV (date,value) atau masukkan data manual. Klik Generate untuk membuat Chart (Chart.js). Setelah generate, gunakan "Edit PNG" untuk menerapkan overlay ke gambar kiri ([...]
...
revertLeftPng.addEventListener('click', ()=>{ const img = leftCanvasWrap.querySelector('img'); if(!img){ alert('No image loaded'); return; } if(img.dataset && img.dataset.original){ img.src = img.data[...]
saveEditedPng.addEventListener('click', ()=>{ const img = leftCanvasWrap.querySelector('img'); if(!img || !img.src){ alert('No image loaded'); return; } const dataUrl = img.dataset.edited || img.src; [...]
...
downloadChartPng.addEventListener('click', ()=>{
  ...
  try { const url = chart.toBase64Image(); const a = document.createElement('a'); a.href = url; a.download = "combined_yoy_chart.png"; document.body.appendChild(a); a.click(); a.remove(); } catch(e){ [...]
});
...
<body>
  <div class="card">
    ...
    <p class="hint">Impor CSV (date,value) atau tambahkan data manual (Year, Value, Count). Setelah Generate, Anda dapat mengedit Year/Value/Count/Diff/% inline, menghapus baris, dan mengekspor seluru[...]
  ...
  tableRows.forEach(r=>{
    ...
    editBtn.addEventListener('click', ()=>{
      ...
      if(!editing){
        ...
        tableRows.push({ year: ny, value: nv, count: isNaN(nc)?0:nc, diff: tdDiff.innerText.trim() === '-' ? null : parseFloat(tdDiff.innerText.replace(/,/g,'')), pct: tdPct.innerText.trim().endsWith([...]
        tableRows.sort((a,b)=> a.year - b.year);
        renderTable();
        recalcStandalone();
      }
...
function recalcStandalone(){
  ...
  if(chartInstance) chartInstance.destroy();
  chartInstance = new Chart(chartEl.getContext('2d'), { type:'line', data:{ labels, datasets:[{ label:'Value', data, borderColor:'rgba(99,102,241,1)', backgroundColor:'rgba(99,102,241,0.12)', fill:tru[...]
}
...
saveCsvBtn.addEventListener('click', ()=>{
  ...
  const csv = rows.map(r=> r.join(',')).join('\n');
  const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); const ts = new Date().toISOString().replace(/[:\-]/g,'').split('.')[0]; a.download = `yoy_stan[...]
});
...
# deploy-clean-fixed.ps1

Fixed Run-GitProcess: sanitize argument list and supply WorkingDirectory to Start-Process.
This prevents Start-Process errors like:
  "Cannot validate argument on parameter 'ArgumentList'. The argument is null, empty, or an element ... contains a null value."

Other behavior unchanged: safe add/commit/rebase/push workflow with helpful logs.

Usage:
  pwsh -ExecutionPolicy Bypass -File .\deploy-clean-fixed.ps1 -RepoDir "C:\Users\ASUS\Desktop\time-series-forecasting" -RemoteUrl "https://github.com/yirassssindaba-coder/Time-Series-Forecasting.git" -AutoSetRemote

Parameters:
  -RepoDir (default: current dir)
  -RemoteUrl (optional)
  -Branch (default: main)
  -AutoSetRemote (switch)
  -ForcePush (switch)
